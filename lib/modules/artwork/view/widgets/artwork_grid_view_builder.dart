...
class ArtworkGridViewBuilder extends StatelessWidget {
  const ArtworkGridViewBuilder({super.key});

  @override
  Widget build(BuildContext context) {
    return Selector<ArtworkViewModel, Tuple2<List<ArtworkModel>, bool>>(
      selector: (_, vm) => Tuple2(vm.artworks, vm.isLoading),
      builder: (context, tuple, child) {
        if (tuple.value2) {
          return ArtworkGridLoaderWidget();
        }
        return ArtworkGridView(
          artworks: tuple.value1,
          onLoadMore: () {
            context.read<ArtworkViewModel>().fetchMoreArtworks();
          },
        );
      },
    );
